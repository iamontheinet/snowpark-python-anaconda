...
def display_data_as_blocks(packages, this_is_a_test=True):
    ...
    for i in range(0,total_packages):
        ...
        if this_is_a_test and i == 9:
            break;

# Set the URL 
repo_anaconda_com_url = 'https://repo.anaconda.com/pkgs/snowflake/'
with st.spinner("Please wait while I load the packages..."):
    packages = get_packages_data(repo_anaconda_com_url)

total_packages = len(packages)

st.header(f"Snowflake Snowpark for Python -- Conda Channel")
